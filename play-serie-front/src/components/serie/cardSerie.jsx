  ...
  return (
    <div className="bg-[#D9D9D9] rounded-md shadow-lg relative" key={props.serie.id}>
      <div className="rounded-md shadow-lg h-full ">
        <Link key={props.serie.id} to={`/serie/${props.serie.id}`}>
          <div key={props.serie.id}>
            <img  width={240} height={200} src={`http://localhost:3001/tmp/${props.serie.image}`} alt="cover" />
          </div>
        </Link>
        <div className="px-2">
      ...
      {managerMode && (
        <Link to={`/serie/edit/${props.serie.id}`} className="absolute top-16 right-24 bg-neutral-700 rounded-full hover:scale-125">
          <img src={edit} width={50} height={50} alt="cover" />
        </Link>
      )}
    </div>);
};
